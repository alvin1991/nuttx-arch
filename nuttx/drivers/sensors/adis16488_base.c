...
static struct adis16488_reg_pair_s g_initial_adis16488_cr_values[] =
{
  ...
  {
    .addr  = ADIS16488_REG_CONFIG,
    .value = 0xC0
  },
  /* Set Control, output sample rate decimation */
  {
    .addr  = ADIS16488_REG_DEC_RATE,
    .value = 2				/* 0   11*/
  },
  /* Set Filter selection BNK0*/
  {
    .addr  = ADIS16488_REG_FILTER_BNK0,
    .value = 0x0000			/* 0x0000  */  /* 0x0924  */
  },
  /* Set Filter selection BNK1*/
  {
    .addr  = ADIS16488_REG_FILTER_BNK1,
    .value = 0x0000			/* 0x0000  */  /* 0x0924  */
  },
};
...
static inline void convert_acc_data(void *dst,void *src)
{
	...
	memcpy(&tmp_low, src+0, 2);
	memcpy(&tmp_out, src+2, 2);


	if(tmp_out >= 0xA81C)
	{
		data_out = (0x10000 - tmp_out) * (-0.0008);
		data_low = (0x10000 - tmp_low) * (-0.0008/65536);
	}
	else
	{
		data_out = tmp_out * 0.0008;
		data_low = tmp_low * 0.0008/65536;
	}

	*(float *)dst = data_out + data_low;
...
static inline void convert_gyro_data(void *dst,void *src)
{
	...
	if(tmp_out >= 0xA81C)
	{
		data_out = (0x10000 - tmp_out) * (-0.02);
		data_low = (0x10000 - tmp_low) * (-0.02/65536);

	}
	else
	{
		data_out = tmp_out * 0.02;
		data_low = tmp_low * 0.02/65536;
	}
...
static inline void convert_temperature_data(void *dst,void *src)
{
	uint16_t tmp = 0;
	float data = 0;
...
//	}
	...
	//temperature
	uint8_t temperature_byte[2];
	ret = adis_read_reg_32(dev, ADIS16488_REG_TEMP_OUT, &temperature_byte);
	if(ret < 0){
		snerr("ERROR: Failed to read angle:%d\n",ret);
	}
...
//	convert_angle_data(&(dev->sdata.angle[Z_AXIS]),&z_ang[2]);
	...
	float temperature;
	convert_temperature_data(&(temperature),&temperature_byte[0]);

	//copy measure data to orb data

	adis16488_data.acc[X_AXIS] =  dev->sdata.acc[X_AXIS];
	adis16488_data.acc[Y_AXIS] = -dev->sdata.acc[Y_AXIS];
	adis16488_data.acc[Z_AXIS] = -dev->sdata.acc[Z_AXIS];

	adis16488_data.gyro[X_AXIS] =  dev->sdata.gyro[X_AXIS];
	adis16488_data.gyro[Y_AXIS] = -dev->sdata.gyro[Y_AXIS];
	adis16488_data.gyro[Z_AXIS] = -dev->sdata.gyro[Z_AXIS];
...
//	memcpy(adis16488_data.angle  ,dev->sdata.angle  ,sizeof(dev->sdata.angle));
	...
	adis16488_data.temp = temperature;
...
{
	int ret;
	uint8_t val = ADIS_GLOB_CMD_SOFTWARE_RESET;

	ret = adis_write_reg_8(dev, ADIS16488_REG_GLOB_CMD, &val);
	if (ret){
...
static uint16_t adis16488_read_firmware_revision(FAR struct adis16488_dev_s *dev,uint16_t* FIRM_REV,uint16_t* FIRM_DM,uint16_t* FIRM_Y)
{
	int ret;

	ret = adis_read_reg_16(dev, ADIS16488_REG_FIRM_REV, FIRM_REV);
	if (ret < 0){
		snerr("ERROR: Failed read device firmware revision.\n");
		return ret;
	}

	ret = adis_read_reg_16(dev, ADIS16488_REG_FIRM_DM, FIRM_DM);
	if (ret < 0){
		snerr("ERROR: Failed read device firmware day/month.\n");
		return ret;
	}

	ret = adis_read_reg_16(dev, ADIS16488_REG_FIRM_Y, FIRM_Y);
	if (ret < 0){
		snerr("ERROR: Failed read device firmware year.\n");
		return ret;
	}
	return *FIRM_REV;
}
...
{
	int ret;
	uint8_t val = ADIS_GLOB_CMD_SELF_TEST;
...
static int adis16488_dvr_open(FAR void *instance_handle, int32_t arg)
{
  ...
  if ((pnpid !=16488))
  ...
  else /* ID matches */
    {
	  ...
	  uint16_t firmware[3] = {0,0,0};
	  vm =  adis16488_read_firmware_revision(priv,&firmware[0],&firmware[1],&firmware[2]);
	  if (vm > 0){
		  sninfo("INFO: Firmware:%04x-%04x-%04x\n", firmware[2],firmware[1],firmware[0]);
	  }
      ...
      /* Choose and apply the initialization sequence */
	  uint8_t reg_num = sizeof(g_initial_adis16488_cr_values)/sizeof(struct adis16488_reg_pair_s);
	  for(int i = 0; i < reg_num; i++){
			ret = adis_write_reg_16(priv, priv->config->initial_cr_values[i].addr, &(priv->config->initial_cr_values[i].value));
			if (ret){
				snerr("ERROR: Failed to initialize register sequence: %d\n", ret);
			}
	  }
	  	uint16_t dec = 0;
		ret = adis_read_reg_16(priv, ADIS16488_REG_FILTER_BNK0,&dec);
		if (ret < 0){
			snerr("ERROR: Failed read ADIS16488_REG_FILTER_BNK0.\n");
			return ret;
		}


	  sninfo("INFO: Initialize register sequence completed[%x][%d].\n",dec,reg_num);
    }
  return OK;
}
